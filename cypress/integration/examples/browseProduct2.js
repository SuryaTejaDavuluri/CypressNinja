...
describe('Tutorials Ninja Shopping', function () {
    ...
    for (let i = 0; i < 8; i++) {
            ...
            let something = device.gadget[i].Model
            something.forEach(function (z) {
                ...
                it('Browse "' + z + '" Product', function () {
                    ...
                    if (device.gadget[i].Product == 'Desktops' || device.gadget[i].Product == 'Components') {
                        cy.get('.nav.navbar-nav .dropdown .dropdown-toggle').each(function (el, index, $list) {

                            let product = $list.eq(index).text()
                            cy.log(product)
                            if (product.includes(device.gadget[i].Product)) {
                                cy.wrap(el).click()
                                if (product == 'Desktops') {
                    ...
                    else if (device.gadget[i].Product == 'Laptop' || device.gadget[i].Product == 'MP3') {
                        cy.get('.nav.navbar-nav .dropdown .dropdown-toggle').each(function (el, index, $list) {
                    ...
                    else if (device.gadget[i].Product == 'Cameras') {
                        cy.get(':nth-child(7) > a').click()
                    }
                    else {
                        cy.get('.nav.navbar-nav li a').contains(device.gadget[i].Product).click({ force: true })
                    }
